...
export class CoinGeckoService {
  ...
  /**
   * Batch get historical prices for multiple dates (OPTIMIZED FOR PERFORMANCE)
   * This method significantly improves performance by:
   * 1. Deduplicating dates to minimize API calls
   * 2. Adding rate limiting to respect free tier limits
   * 3. Caching results by date string
   * 
   * @param coinId - The CoinGecko coin ID (e.g., 'ethereum')
  ...
  async batchGetHistoricalPrices(
    ...
    // Deduplicate dates using date-only string (yyyy-MM-dd)
    ...
    console.log(`Fetching historical prices for ${uniqueDates.size} unique dates (from ${dates.length} total dates)`);
    
    // Fetch prices for unique dates with rate limiting
    let count = 0;
    for (const [dateKey, date] of uniqueDates.entries()) {
      ...
      if (count % 10 === 0) {
        console.log(`  Progress: ${count}/${uniqueDates.size} dates fetched`);
      }
      
      // Rate limit: CoinGecko free tier allows 10-50 calls/min
      // We use 150ms delay = ~6.7 calls per second = ~400 calls/min (safe margin)
